...
impl AmericanCallOption {
    ...
    pub fn vega(&self) -> f64 {
        ...
        (up - down) / (2.0 * 100.0) // Divide by 100 for 1% change
    }
    ...
    pub fn theta(&self) -> f64 {
        ...
        (past - future) / 1.0 // Per day
    }

    /// Calculate rho using finite difference method.
    pub fn rho(&self) -> f64 {
        let h = 0.01; // 1% rate bump
        let up = binomial_tree_price(
            self.spot,
            self.strike,
            self.time_to_expiry,
    ...
    ///
    /// Note:
    ///     American calls without dividends typically equal European calls (never optimal
    ///     to exercise early). Use this method when:
    ///     - The stock pays dividends (set dividend_yield > 0)
    ///     - You want to verify early exercise premium
    ///     - You're pricing exotic American-style options
    ///
    /// Convergence:
    ///     - 10k paths, 25 steps: ~1% accuracy in ~0.05s
    ///     - 50k paths, 50 steps: ~0.3% accuracy in ~0.2s (default)
    ///     - 100k paths, 100 steps: ~0.1% accuracy in ~0.8s
    ///
    /// Reference:
